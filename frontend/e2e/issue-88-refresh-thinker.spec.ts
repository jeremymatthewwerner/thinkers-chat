...
test.describe('Issue #88: Refresh thinker suggestion fails', () => {
  ...
  test('refresh suggestion should complete without "load failed" error', async ({ page }) => {
    // Set up network request monitoring
    const apiCalls: { url: string; startTime: number; endTime?: number; status?: number; error?: string }[] = [];
    ...
    page.on('requestfailed', request => {
      if (request.url().includes('/api/thinkers/suggest')) {
        const call = apiCalls.find(c => c.url === request.url() && !c.endTime);
        if (call) {
          call.endTime = Date.now();
    ...
    const spinnerAppeared = await page.locator('[data-testid="refresh-suggestion"] svg.animate-spin').isVisible().catch(() => false);
    ...
    const errorMessage = page.locator('[data-testid="new-chat-modal"] .text-red-600, [data-testid="new-chat-modal"] .text-red-400');
    ...
    try {
      await Promise.race([
        ...
        (async () => {
          ...
          for (let i = 0; i < 40; i++) {
            const currentSpinner = await page.locator('[data-testid="refresh-suggestion"] svg.animate-spin').first().isVisible().catch(() => false);
            if (!currentSpinner) {
    ...
    for (const call of newCalls) {
      if (call.endTime && call.startTime) {
        ...
        if (duration < 500 && call.status === 200) {
          console.warn(`SUSPICIOUS: API response came back in ${duration}ms - may indicate mock data`);
        }
      }
    }
    ...
    const finalName = await firstSuggestion.locator('div.font-medium').textContent();
    expect(finalName).toBeTruthy();
    console.log(`Final suggestion name: ${finalName}`);
  });

  test('multiple rapid refresh clicks should not cause errors', async ({ page }) => {
    ...
    await expect(page.locator('h2', { hasText: 'Select Thinkers' })).toBeVisible({ timeout: 30000 });
    await expect(page.getByTestId('thinker-suggestion').first()).toBeVisible({ timeout: 30000 });
  ...
  test('suggestion refresh API timing should be reasonable', async ({ page }) => {
    ...
    if (apiCallStart && apiCallEnd) {
      ...
      if (duration < 500) {
        console.warn(`SUSPICIOUS: Response too fast (${duration}ms) - may not be hitting real Claude API`);
      }

      // If it's > 30s, there's a timeout issue
      if (duration > 30000) {
        console.warn(`WARNING: Response too slow (${duration}ms) - may indicate timeout issue`);
      }
    } else {
